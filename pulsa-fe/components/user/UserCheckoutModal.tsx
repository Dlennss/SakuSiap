...
export function UserCheckoutModal({
  ...
  function handleDownloadProof() {
    void (async () => {
      ...
      doc.setFontSize(11);
      doc.text("SakuSiap - QRIS Payment Receipt", centerX, 61, { align: "center" });
      doc.setFontSize(10);
      ...
      doc.setFont("helvetica", "normal");
      doc.text("Dokumen ini dibuat otomatis oleh sistem SakuSiap dan valid sebagai bukti pembayaran.", 56, finalY + 58);
            ...
            <div className={`grid gap-3 ${showQRCode ? "grid-cols-2" : "grid-cols-1"}`}>
              {showQRCode ? (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex h-11 w-full items-center justify-center rounded-2xl bg-linear-to-r from-[#21714c] to-[#21714c] px-4 text-sm font-semibold text-white shadow-[0_8px_20px_rgba(15,111,203,0.24)]"
                >
                  Buka QRIS
                </a>
              ) : null}
          ...
          <form id="guest-checkout-form" onSubmit={handleSubmit} className="mt-5 space-y-4">
            ...
            {needsTopup ? (
              <button
                ...
                className="inline-flex h-13 w-full items-center justify-center rounded-2xl bg-[#21714c] px-5 text-xs font-black text-white shadow-[0_10px_20px_rgba(4,120,87,0.22)] transition hover:bg-[#036b4d]"
              >
                Isi Saldo
              </button>
            ) : (
              <button
                type="submit"
                disabled={loading}
                className="inline-flex h-13 w-full items-center justify-center rounded-2xl bg-[#1c4032] px-11 text-xs font-black text-white shadow-[0_10px_20px_rgba(5,46,38,0.18)] transition group-hover:bg-[#21714c]"
              >

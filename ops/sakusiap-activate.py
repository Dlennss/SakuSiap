#!/usr/bin/python3
"""Activate fixed SakuSiap staging paths; roll back both units if health checks fail."""
import fcntl
import os
...
BASE = Path('/var/lib/syslog-ng/fadlanpulsa/deployment/sakusiap')
STAGE = Path('/var/lib/syslog-ng/sakusiap')
...
with open('/run/lock/sakusiap-deploy.lock', 'w') as lock:
    fcntl.flock(lock, fcntl.LOCK_EX)
    commit = (STAGE / 'frontend/DEPLOY_COMMIT').read_text().strip()
    if not re.fullmatch('[0-9a-f]{40}', commit):
        raise SystemExit('Invalid release commit')
    for name in ['server.js', '.next/BUILD_ID', 'public/deploy-version.txt', 'public/sakusiap-assets/00_full_screen/dashboard_full.png']:
        if not (STAGE / 'frontend' / name).is_file():
            raise SystemExit('Incomplete frontend: ' + name)
    binary = STAGE / 'pulsa-be/sakusiap-api'
    ...
    shutil.copyfile(binary, release / 'sakusiap-api')
    os.chmod(release / 'sakusiap-api', 0o755)
    (frontend / '.next/cache').mkdir(exist_ok=True)
    units = {
        'sakusiap-fe': '[Service]\nWorkingDirectory=' + str(frontend) + '\nBindPaths=\nBindPaths=/var/cache/sakusiap-fe:' + str(frontend / '.next/cache') + '\n',
        'sakusiap-be': '[Service]\nExecStart=\nExecStart=' + str(release / 'sakusiap-api') + '\n',
    }
    previous = {}
    try:
        ...
        run('systemctl', 'restart', 'sakusiap-be', 'sakusiap-fe')
        for attempt in range(30):
            try:
                fetch('http://127.0.0.1:8105/health')
                actual = fetch('http://172.22.0.1:33031/deploy-version.txt').strip()
                fetch('http://172.22.0.1:33031/')
                if actual == commit:
                    print('SakuSiap active: ' + commit, flush=True)
                    break
            except Exception:
                pass
            time.sleep(2)
        else:
            raise RuntimeError('SakuSiap health/release verification failed')
    except Exception:
